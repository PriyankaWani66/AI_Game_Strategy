...
class BFSActManStrategy(MoveStrategy):
    def __init__(self, dungeon, monsters, act_man):
        self.dungeon = dungeon
        self.monsters = monsters
        self.act_man = act_man
    ...
    def create_copy(self, dungeon, monsters, act_man):
        ...
        act_man_copy.move_strategy = BFSActManStrategy(dungeon_copy, monsters_copy, act_man_copy)
    ...
    def get_next_action(self):
        ...
        queue = deque([(copy.deepcopy(self.act_man.pos), 
        ...
        max_monsters_died = 0
        while queue:
            pos, score, fire_attempts, state_dungeon, state_monsters, state_act_man, actions, dungeon_state_sequence = queue.popleft()
            ...
            if len(actions) == 7:
                ...
                continue  # Don't explore further if the action sequence reaches 7 turns
            ...
            for direction in self.move_strategy.deltas_for_numeric_directions:
                ...
                if move_result['is_alive']:
                    ...
                    if updated_monsters is not None:  # Only add the state to the queue if Act-Man survives
                        ...
                        queue.append((new_pos, score - 1 + monster_Score, fire_attempts, dungeon_copy, monsters_copy, act_man_copy, new_actions, new_dungeon_state_sequence))
            ...
            if fire_attempts > 0:
                for direction in self.fire_strategy.deltas_for_literal_directions:
                    ...
                    if updated_monsters is not None: # Only add the state to the queue if Act-Man survives
                        ...
                        new_actions = actions + [direction]
                        new_dungeon_state_sequence = dungeon_state_sequence + [dungeon_copy]
                        queue.append((pos, score - 20 + fire_result["score"] + monster_Score, fire_attempts - 1, dungeon_copy, monsters_copy, act_man_copy, new_actions, new_dungeon_state_sequence))
...
class ActManGame: # Actual game
    ...
    def load_dungeon(self, input_file):
        ...
        self.act_man.move_strategy = BFSActManStrategy(self.dungeon, self.monsters, self.act_man)
    ...
    def play_game(self, output_file):
        action_sequence, best_dungeon_state_sequence, best_score = self.act_man.move_strategy.get_next_action()
          # Get the sequence of actions from BFS   
    ...
    # Write the final game state to the output file
